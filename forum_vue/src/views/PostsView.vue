...
export default {
  methods: {
    page(currentPage) {
      if (this.search === "" || this.search == null) {
        this.$axios
          .get("/post/selectAll/" + currentPage + "/10/" + this.DESC)
          .then((resp) => {
            if (resp.data != "") {
    ...
    searchPost() {
      ...
      if (this.search != "" && this.search != null) {
        this.$axios
          .get("/post/selectAll/" + this.search + "/1/10/" + this.DESC)
          .then((resp) => {
            if (resp.data != "") {
              this.tableData = resp.data;
        ...
        this.$axios.get("/post/total/" + this.search).then((resp) => {
  ...
  created() {
    ...
    this.$axios.get("/post/total").then((resp) => {
      this.total = resp.data;
